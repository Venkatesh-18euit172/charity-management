...
<html>
	...
	<body id="body-pd">
		<%
		 ArrayList<Request> req =new RequestDao().getRequest();	
		for (Request Req: req) {
		...
		<div style="margin: 30px 2%; width:45%; display: inline-block; border-radius: 10px;  background: linear-gradient(to bottom, #ffff66 0%, #66ff99 100%); text-align: center;">
			<div class="event" style="width: 100%; text-align: center; padding: 10px 0;">
				...
				<td style= "width : 100px;border: 1px ;border-collapse: collapse;border-color: #FFFF99;background-color: #FFC0CB;padding: 10px;"> 
				<button style="color: red; border-radius: 5px;border-width: 0px;background-color: #ffffff;padding: 7px;"><a href='DonationList?id=<%=Req.getId()%>'>View Donation</a></button> </td>
				<td style= "width : 100px;border: 1px ;border-collapse: collapse;border-color: #FFFF99;background-color: #FFC0CB;padding: 10px;"> 
	            <button style="color: red; border-radius: 5px;border-width: 0px;background-color: #ffffff;padding: 7px;" onclick='display("<%=Req.getRequest_details()%>")' > Change Request </button></td>
			</div>
		</div>
		<% }%>
